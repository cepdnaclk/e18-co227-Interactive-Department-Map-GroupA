...
function searchTable(value,data){
    ...
    for(var i=0;i<data.length;i++){
        ...
        if(data[i].category==2){
            var lec=data[i].staff.toLowerCase()
            if(lec.includes(value)){
...
function buildTable(data){
    ...
    for (var i = 0; i < data.length; i++){
        ...
        if(data[i].category==2){
            var row = `<tr>
            <td>${'<p id="num">'+data[i].name+'</p><ul id="abc"><li><small>'+data[i].staff+'</small></li></ul>'}</td>
             </tr>`
        table.innerHTML += row
        }
        else{
            var row = `<tr>
                        <td>${'<p id="num">'+data[i].name+'</p>'}</td>
                     </tr>`
        table.innerHTML += row
        }
...
var lastID = 0;        //BuldingID of the last showPath function call
...
function showPath(buildingID){
    if(itemSelected){   //If item has been selected
        ...
        transparentMaterialForPanelsArray[buildingID].opacity=0;
        ...
        // }
            ...
            transparentMaterialForPanelsArray[buildingID].opacity=0.6;
            for(var i=0;i<(my_json[buildingID].path).length;i++){
            NavigatingMaterialArray[my_json[buildingID].path[i]].opacity = 1;   //Show thw path (using bars)
        }
        lastID = buildingID;        //Set the LastID
        itemSelected = !itemSelected;
    }    
}
function searchingNav(){   
    ...
    var value= $(this).val()
    data = searchTable(value,my_json)
    buildTable(data);

    $(document).ready(function(){
    $('table tr #num').click(function(){
        var myText=$(this).text();
        var newIndex=-1;
        var filteredObj = my_json.find(function(item, i){
         if(item.name === myText){
             index = i;
             roomInfo(index);
             showPath(index);
